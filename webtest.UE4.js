...
console.log("Emscripten version: E:/unreal/UE_4.23/Engine/Extras/ThirdPartyNotUE/emsdk/emscripten/1.38.31");
console.log("Emscripten configuration: E:/unreal/UE_4.23/Engine/Intermediate/Build/HTML5/.emscripten");
...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_webtest', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202306231623, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = [ '../../../webtest/webtest.uproject', '-stdout', ];
...
function resizeCanvas(aboutToEnterFullscreen) {
	...
	Module[ 'canvas' ].style.height = mainArea.style.height = 100 + 'vh';
...
function getIDBRequestErrorString(req) {
	try {
		return req.error ? ('IndexedDB ' + req.error.name + ': ' + req.error.message) : req.result;
...
$(document).ready(function () {
	...
	function withIndexedDB(db) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('webtest.data.js'));
		...
		var dataDownload =
			/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
						fetchOrDownloadAndStore(db, Module.locateFile('webtest.data')).then(function(dataBlob) {
							return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
								Module['preloadedPackages'] = {};
								Module['preloadedPackages'][Module.locateFile('webtest.data')] = dataArrayBuffer;
			...
			*/
			...
			fetchOrDownloadAndStore(db, Module.locateFile('webtest.data'), 'arraybuffer').then(function (dataArrayBuffer) {
				Module[ 'preloadedPackages' ] = {};
				Module[ 'preloadedPackages' ][ Module.locateFile('webtest.data') ] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
